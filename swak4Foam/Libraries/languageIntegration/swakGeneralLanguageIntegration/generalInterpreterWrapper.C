...
Contributors/Copyright:
    2011-2020 Bernhard F.W. Gschaider <bgschaid@hfd-research.com>
...
void generalInterpreterWrapper::write(Ostream &os) const
{
    ...
    if(dict_.found(errorMode)) {
        os.writeKeyword(errorMode)
            << word(dict_.lookup(errorMode)) << token::END_STATEMENT << nl;
    }
...
void generalInterpreterWrapper::dictionariesToInterpreterStructs() {
    ...
    forAll(who,i) {
        ...
        if(dictionariesIntoMemory_.isDict(name)) {
            ...
            dictName=word(sub.lookup("name"));
            regionName=word(sub.lookup("region"));
            me=false;
        } else {
            dictName=word(dictionariesIntoMemory_.lookup(name));
        }
        const objectRegistry &obr=
            me
            ? obr_
...
void generalInterpreterWrapper::interpreterStructsToDictionaries() {
    ...
    forAll(who,i) {
        ...
        if(memoryIntoDictionaries_.isDict(name)) {
            ...
            dictName=word(memoryIntoDictionaries_.lookup(name));
        }
        const objectRegistry &obr=
            me
            ? obr_
...
void generalInterpreterWrapper::setReferences()
{
    ...
    forAll(referenceSpec_,specI) {
        ...
        const word name(spec.lookup("name"));
        const word type(spec.lookup("type"));
        if(
            type!="internalField"
            &&
            type!="patchField"
        ...
        if(type=="internalField") {
            word fieldName(spec.lookup("fieldName"));
            ...
            word fieldName(spec.lookup("fieldName"));
            word patchName(spec.lookup("patchName"));
