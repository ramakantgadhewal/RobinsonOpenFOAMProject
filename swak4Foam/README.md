...
-   2006-2021 Bernhard F.W. Gschaider <bgschaid@hfd-research.com>
...
-   2016-2020 Mark Olesen <Mark.Olesen@esi-group.com>
...
## 2021-05-31 - version number : 2021.05
...
#### Support for OpenFOAM v2012

Compiles and tested with version v2012 of the ESI Fork


#### Hotfix: compilation on OpenFOAM v2106

The version compiles on OpenFOAM v2106 but the functionality is
completely untested
...
#### Compile script fails if no OpenFOAM-version is activated

Now the script ends with a specific error message instead of
failing later with an error message that does not directly point
to this problem


#### Avoid problems with distros that don't have a `python` utility

This is usually caused by some Linux distros (most prominently
Ubuntu 20.04) that install `python2` and/or `python3` but none of
those can be called as `python`.

These distros usually supply a package that links `python` to
...
#### Selecting the method that is executed for function objects based on `simpleFunctionObject`

These function objects now have two optional parameters `noWrite`
and `noExcute` skip execution if the functionality is called via
`execute()` or `write()`
